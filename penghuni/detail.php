<?php
  ...
  if (isset($_POST['submit'])){
    ...
    $now = date_create(strftime("%Y-%m-%d"));
    $mulai = date_create($_POST['tanggal']);
  	...
  	$harga = 0;
    	...
    	if ($diff->format("%R") == '-'){
      		...
      		if((int)($diff->format("%a")) > 7) {
        		...
        		mysqli_query($koneksi,"INSERT INTO booking (id_booking,id_kamar,username, tanggal_booking, harga, validasi, bayar, aktif) VALUES (NULL,'$id','$user','$tanggal',$harga,'belum','lunas','true')");
        		$id = mysqli_insert_id($koneksi);
            	mysqli_query($koneksi,"UPDATE kamar SET status='tidak tersedia' WHERE id_kamar=$id ");
        		echo "<script>
                    alert('Booking berhasil !');
                    document.location.href = 'kuitansi.php?id=$id';
                </script>";
      		}
    	}
  	}
                ...
                <div class="card">
                  ...
                  <div class="card-body" >
                    ...
                    <div class="row g-3 align-items-center">
                        ...
                        <div class="col-auto">
                            <input type="date" required name="tanggal" id="inputPassword6" class="form-control" placeholder="DD/MM/YYYY">
                        </div>
                    </div>
                    <br>
                    <?php if ($data['status'] == "tidak tersedia"){ ?>
                      <button disabled class="btn btn-primary me-md-2" href="">Booking</button>
                    <?php } else { ?>
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-6">
          ...
          <div style="box-shadow: 10px 0 5px -2px gray;" data-aos="fade-right">
